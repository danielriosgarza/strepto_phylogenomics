...
def patric_and_db_ids(file, genus):
    with open(file) as f:
        ...
        with open(os.path.join(p.parents[0], 'files', genus+'_patric_id_with_database_id.tsv'), 'w') as f2:
...
streptoinput = os.path.join(p.parents[0], 'files', '06012020_streptococcus_genomes_quality.tsv')

patric_and_db_ids(lactoinput, 'lactococcus')
patric_and_db_ids(floriinput, 'floricoccus')
patric_and_db_ids(streptoinput, 'streptococcus')